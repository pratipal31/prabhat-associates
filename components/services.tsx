    ...
    <section id="services" className="py-16 sm:py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-primary mb-4 leading-tight">Our Services</h2>
        </div>

        {/* 2 cols on mobile, 4 cols on desktop — smaller cards on mobile */}
        <div ref={ref} className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
          {services.map((service, index) => (
            ...
            >
              <div className="group relative w-full aspect-square rounded-lg sm:rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 cursor-pointer">
                ...
                <div className="absolute top-0 left-0 right-0 flex flex-col items-center pt-3 sm:pt-5 px-2 sm:px-4 text-center">
                  <h3 className="text-white font-bold text-[10px] sm:text-xs md:text-sm leading-snug drop-shadow-lg tracking-wide group-hover:scale-105 transition-all duration-500">
                    {service.title}
                  </h3>
                  <div className="mt-1.5 sm:mt-2 h-0.5 w-0 group-hover:w-8 sm:group-hover:w-10 bg-white transition-all duration-500 rounded-full"></div>
                </div>
